...
class DishTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.cook = Cook.objects.create_user(username="testcook", password="testpass", first_name="John", last_name="Doe")
        self.ingredient = Ingredient.objects.create(name="Tomato")

    def test_dish_list_view(self):
        dish = Dish.objects.create(name="Spaghetti", description="Delicious pasta", price=12.99, dish_type=self.dish_type)
    ...
    def test_dish_list_view_with_query(self):
        ...
        response = self.client.get(reverse('core:dish-list') + "?query=Spaghetti")
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, dish.name)
    ...
    def test_dish_create_view(self):
        ...
        response = self.client.post(reverse("core:dish-create"), {
            "name": "New Dish",
    ...
    def test_dish_update_view(self):
        ...
        response = self.client.post(reverse('core:dish-update', args=[dish.pk]), {
            'name': 'Updated Spaghetti', 'description': 'Yummy pasta', 'price': 15.99, 'dish_type': self.dish_type.id,
            'cooks': [self.cook.id], 'ingredients': [self.ingredient.id]
        })
    ...
    def test_dish_delete_view(self):
        ...
        self.client.login(username="testcook", password="testpass")
        response = self.client.post(reverse('core:dish-delete', args=[dish.pk]))
        self.assertEqual(response.status_code, 302)
