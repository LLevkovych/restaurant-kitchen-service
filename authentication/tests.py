...
class AuthenticationTests(TestCase):
    ...
    def test_register_view_post(self):
        response = self.client.post(reverse("authentication:register"), self.new_user_data)
    ...
    def test_login_logout_flow(self):
        login = self.client.post(reverse("authentication:login"), self.credentials)
    ...
    def test_profile_update(self):
        ...
        response = self.client.post(reverse("authentication:profile-update"), update_data)
        self.assertEqual(response.status_code, 302)
    ...
    def test_change_password(self):
        ...
        response = self.client.post(reverse("authentication:change-password"), data)
        self.assertEqual(response.status_code, 302)

        self.client.logout()
        login_old = self.client.login(username="testcook", password="securepass123")
        self.assertFalse(login_old)

        login_new = self.client.login(username="testcook", password="NewSecret456!")
